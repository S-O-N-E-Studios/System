...
const FILE_MIME_PATTERNS = {
  document: /^(application\/pdf|application\/msword|application\/vnd\.openxmlformats-officedocument\.wordprocessingml\.document|application\/vnd\.ms-excel|application\/vnd\.openxmlformats-officedocument\.spreadsheetml\.sheet|text\/csv|application\/acad|application\/x-acad|application\/dwg|image\/vnd\.dwg|application\/octet-stream)$/i,
...
const registerFileSchema = Joi.object({
  ...
  stage: Joi.number().integer().min(0).max(10).allow(null).default(null),
...
const uploadUrlSchema = Joi.object({
  projectId: Joi.string().hex().length(24).required(),
  stage: Joi.number().integer().min(0).max(10).allow(null).optional(),
