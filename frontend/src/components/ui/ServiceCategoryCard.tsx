  ...
  return (
    <div className="bg-[var(--bg-surface)] border border-[var(--border-default)] overflow-hidden">
      <button
        ...
        className="w-full flex flex-wrap items-center gap-4 p-6 text-left hover:bg-[var(--accent-sand-glow)] transition-colors"
      >
          ...
          <table className="w-full table-fixed">
            <thead>
              <tr style={{ background: 'var(--table-header-bg)' }}>
                <th className="text-table-header text-left px-4 py-2 truncate">Project</th>
                <th className="text-table-header text-left px-4 py-2 truncate">Local Municipality</th>
                <th className="text-table-header text-right px-4 py-2 truncate">Budget</th>
                <th className="text-table-header text-left px-4 py-2 truncate">Stage</th>
                <th className="text-table-header text-left px-4 py-2 truncate">Status</th>
              </tr>
            </thead>
            <tbody>
              {projects.map((p, i) => (
                ...
                >
                  <td className="px-4 py-2 min-w-0 truncate">
                    <Link
                      to={`/${tenantSlug}/projects/${p.id}`}
                      className="block truncate text-[0.82rem] font-medium text-[var(--accent-periwinkle)] hover:underline"
                    >
                      {p.name}
                    </Link>
                  </td>
                  <td className="px-4 py-2 text-[0.78rem] text-[var(--text-muted)] min-w-0 truncate">
                  ...
                  <td className="px-4 py-2 text-[0.78rem] min-w-0 truncate">
